...
Config.JobGroups = {
    ...
    { id = 'organserver', label = 'Organ Services',         jobs = {'taxi', 'mechanic', 'medic', 'weazel'} },
}

-- Change Job (Branch) keeps the player's CURRENT numeric grade when switching jobs
-- (e.g. grade 8 in CID -> grade 8 in FBI). Make sure every job in the same
-- Config.JobGroups branch has the same number of grades, or the new job may show
-- a blank grade label for ranks that don't exist on it.