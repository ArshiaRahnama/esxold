...
TriggerEvent('esx_society:registerSociety', 'marshal', 'Marshal', 'society_marshal', 'society_marshal', 'society_marshal', {type = 'public'})
...
RegisterCommand('findnumber_marshal', function(source, args, users)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.job.name == "marshal" or xPlayer.job.name == "sheriff"  or xPlayer.job.name == "fbi" then
        if args[1] then
		...
		TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', " ^0Shoma marshal nistid!"} })
    end
end)
...
ESX.RegisterServerCallback('esx_marshaljob:buy', function(source, cb, amount)
	...
	TriggerEvent('esx_addonaccount:getSharedAccount', 'society_marshal', function(account)
...
RegisterServerEvent('esx_marshaljob:saundplay')
AddEventHandler('esx_marshaljob:saundplay', function(soundFile, soundVolume, x, y, plate, IsDistress)
    local source = source
    ...
    if not sentreq[source] then
        ...
        if IsDistress then
            ...
            TriggerClientEvent('esx_marshaljob:sendbackuptext', source, text)
            

            for i=1, #xPlayers do
                local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
                if xPlayer and xPlayer.source ~= source and (xPlayer.job.name == "marshal" or xPlayer.job.name == "sheriff" or xPlayer.job.name == "fbi"  or xPlayer.job.name == "mt") then
...
RegisterCommand('resp_marshal', function(source, args, rawCommand)
    local panicId = tonumber(args[1])
    local xPlayer = ESX.GetPlayerFromId(source)

    if xPlayer and xPlayer.job.name == "marshal" or xPlayer.job.name == "sheriff" or xPlayer.job.name == "fbi" or xPlayer.job.name == "mt" then
        if panicId and panicreqx[panicId] and panicreqy[panicId] then
		
            TriggerClientEvent('esx_marshaljob:markPanicLocation', source, panicreqx[panicId], panicreqy[panicId], panicreqID[panicId])
            ...
            for i = 1, #xPlayers do
                ...
                if targetPlayer and targetPlayer.job.name == "marshal" then
        ...
        TriggerClientEvent('esx:showNotification', source, '~r~Shoma Marshal Nistid!')
    end
end, false)
...
RegisterCommand('createunit_marshal', function(source, args)
	if not args[1] then
	...
	if xPlayer.job.name == "marshal" or xPlayer.job.name == "sheriff" or xPlayer.job.name == "mt" or xPlayer.job.name == "fbi" then
		local identifier = xPlayer.identifier

		if units[identifier] == nil and not IsPlayerInAnyUnit_marshal(identifier) then
			...
			if callsigns[uidentifier] == nil then
				...
				TriggerClientEvent('esx_marshaljob:notifyp', -1, " Vahed ^2" .. uidentifier .. "^0 Tavasot ^3" .. string.gsub(xPlayer.name, "_", " ") .. "^0 sakhte shod!", xPlayer.job.name)
			else
			...
			if not IsPlayerInAnyUnit_marshal(identifier) then
...
RegisterCommand('delunit_marshal', function(source, args)
	if not args[1] then
		TriggerClientEvent('chatMessage', source, "[ System ] ", {255, 0, 0}, "Syntax Vared Shode Eshtebah Ast")
		return
	end
		...
		if callsigns[uidentifier] ~= nil then
			...
			if callsigns[uidentifier].job == xPlayer.job.name then
				if xPlayer.job.grade >= 18 then 
					...
					if TableLength_marshal(units[identifier].members) > 0 then
					...
					TriggerClientEvent('esx_marshaljob:notifyp', -1, " Vahed ^2" .. uidentifier .. "^0 Tavsot ^3" .. Name .. "^0 Monhal Shod!", job)
				else
...
RegisterCommand('renameunit_marshal', function(source, args)
	...
	if xPlayer.job.name == "marshal" or xPlayer.job.name == "sheriff" or xPlayer.job.name == "mt" or xPlayer.job.name == "fbi" then
		...
		if units[identifier] ~= nil then
			...
			TriggerClientEvent('esx_marshaljob:notifyp', -1, " Vahed ^2" .. csign .. "^0 be ^3" .. uidentifier .. "^0 taghir yaft!", job)
...
RegisterCommand('disbanunit_marshal', function(source)
	...
	if xPlayer.job.name == "marshal" or xPlayer.job.name == "sheriff" or xPlayer.job.name == "mt" or xPlayer.job.name == "fbi" then
		...
		if units[identifier] ~= nil then
			...
			TriggerClientEvent('esx_marshaljob:notifyp', -1, " Vahed ^2" .. csign .. "^0 monhal shod!", xPlayer.job.name)
...
RegisterCommand('units_marshal', function(source, args)
	...
	if xPlayer.job.name == "marshal" or xPlayer.job.name == "sheriff" or xPlayer.job.name == "mt" or xPlayer.job.name == "fbi" then
		local identifier = xPlayer.identifier
		local job = xPlayer.job.name
		if TableLength_marshal(callsigns) > 0 then
			if args[1] == "all" then
				if xPlayer.job.name == "fbi" then
					for k,v in pairs(callsigns) do
						TriggerClientEvent('chatMessage', source, "[ Info ] : ", {226, 239, 93}, "UNIT ^3'" .. k .. "'^0,   Leader: ^2" .. v.name .. "^0, Members: ^1" .. TableLength_marshal(units[v.owner].members))
					end
				else
					TriggerClientEvent('chatMessage', source, "[ System ] ", {255, 0, 0}, "Shoma dastresi kafi baraye in dastor ra nadarid!")
				end
...
RegisterCommand('joinunit_marshal', function(source, args)
	if not args[1] then
		TriggerClientEvent('chatMessage', source, "[ System ] ", {255, 0, 0}, "Syntax Vared Shode Eshtebah Ast")
		return
	end
	...
	if xPlayer.job.name == "marshal" or xPlayer.job.name == "sheriff" or xPlayer.job.name == "mt" or xPlayer.job.name == "fbi" then
		...
		if units[identifier] == nil and not IsPlayerInAnyUnit_marshal(identifier) then
			...
			if callsigns[uidentifier] ~= nil then
				...
				TriggerClientEvent('esx:setcallsign', source, uidentifier)
				TriggerClientEvent('esx_marshaljob:notifyp', -1, "^3" ..string.gsub(xPlayer.name, "_", " ") .. "^0 Be Vahed ^2" .. uidentifier .. "^0 molhagh shod!", xPlayer.job.name)
			else
		...
		TriggerClientEvent('chatMessage', source, "[ System ] ", {255, 0, 0}, "Shoma marshal nistid")
	end
end, false)

RegisterCommand('leaveunit_marshal', function(source)
	...
	if xPlayer.job.name == "marshal" or xPlayer.job.name == "sheriff" or xPlayer.job.name == "mt" or xPlayer.job.name == "fbi" then
		...
		if units[identifier] == nil and IsPlayerInAnyUnit_marshal(identifier) then

			for k,v in pairs(units) do
				if v.members[identifier] then
					TriggerClientEvent('esx_marshaljob:notifyp', -1, "^3" ..string.gsub(xPlayer.name, "_", " ") .. "^0 AzVahed ^2" .. v.callsign .. "^0 kharej shod!", xPlayer.job.name)
...
local function IsMarshalForUnit(jobname)
	return jobname == "marshal" or jobname == "sheriff" or jobname == "mt" or jobname == "fbi"
end
...
AddEventHandler('Unit:RobAlarm_marshal', function(Name)
	...
	TriggerClientEvent('esx_marshaljob:notifyp', -1, "Alarm robbery dar ^2" .. Name .. "^0 jahat Accept ^3/acceptrob " .. RobberyCode, "marshal")
	SetTimeout(10 * 60000, function()
		Robbs[RobberyCode] = nil
	end)
end)

RegisterCommand('acceptrob_marshal', function(source, args)
	local xPlayer = ESX.GetPlayerFromId(source)

	if IsMarshalForUnit(xPlayer.job.name) then
		...
		if code and type(Robbs[code]) == 'table' then
			if Robbs[code].Accept == false then
				...
				TriggerClientEvent('esx_marshaljob:notifyp', -1, "Robbery ^2" .. Robbs[code].Name .. "^0 tavasote ^3" .. string.gsub(xPlayer.name, "_", " ") .. "^0 (" .. string.upper(xPlayer.job.name) .. ") accept shod", "marshal")
			else
...
function CheckRob_marshal(RobberyCode)
	return type(Robbs[RobberyCode]) == 'table' and Robbs[RobberyCode].Accept or false
end
exports('CheckRob_marshal', CheckRob_marshal)

function TableLength_marshal(table)
...
function IsPlayerInAnyUnit_marshal(identifier)
	for k,v in pairs(units) do
		if v.members[identifier] then
			return true
		end
...
-- RegisterCommand('bc_marshal', function(source, args)
-- 	local xPlayer = ESX.GetPlayerFromId(source)

-- 	if xPlayer.job.name == 'marshal' then
-- 		local dutytext = nil
-- 		local job = xPlayer.job.name
...
-- RegisterCommand('rsr_marshal', function(source, args)
-- 	local xPlayer = ESX.GetPlayerFromId(source)
