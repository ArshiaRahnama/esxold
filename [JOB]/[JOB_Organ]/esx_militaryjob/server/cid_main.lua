...
if Config_cid.MaxInService ~= -1 then
	TriggerEvent('esx_service:activateService', 'CID', Config_cid.MaxInService)
end
...
RegisterServerEvent('esx_cidjob:getStockItem')
...
RegisterCommand('findnumber_cid', function(source, args, users)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.job.name == "cid" or xPlayer.job.name == "sheriff" then
        if args[1] then
		...
		TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', " ^0Shoma cid nistid!"} })
    end
end)



ESX.RegisterServerCallback('esx_cidjob:getVehicleInfos', function(source, cb, plate)
...
ESX.RegisterServerCallback('esx_cidjob:getVehicleFromPlate', function(source, cb, plate)
	MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE plate = @plate', {
		['@plate'] = plate
	}, function(result)
...
ESX.RegisterServerCallback('esx_cidjob:getArmoryWeapons', function(source, cb)

	TriggerEvent('esx_datastore:getSharedDataStore', 'society_cid', function(store)
...
ESX.RegisterServerCallback('esx_cidjob:addArmoryWeapon', function(source, cb, weaponName, removeWeapon)
...
ESX.RegisterServerCallback('esx_cidjob:buyArmoryWeapon', function(source, cb, weaponName, removeWeapon, tedad)
...
ESX.RegisterServerCallback('esx_cidjob:removeArmoryWeapon', function(source, cb, weaponName)
...
ESX.RegisterServerCallback('esx_cidjob:getStockItems', function(source, cb)
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_cid', function(inventory)
		cb(inventory.items)
	end)
end)
...
ESX.RegisterServerCallback('esx_cidjob:buyArmoryItem', function(source, cb, weaponName, removeWeapon, tedad)
...
ESX.RegisterServerCallback('esx_cidjob:getPlayerInventory', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('esx_phone:removeNumber', 'cid')
	end
end)

RegisterServerEvent('esx_cidjob:message')
AddEventHandler('esx_cidjob:message', function(target, msg)
...
RegisterServerEvent('esx_cidjob:saundplay')
AddEventHandler('esx_cidjob:saundplay', function(soundFile, soundVolume, x, y, plate, IsDistress)
    local source = source
    ...
    if not sentreq[source] then
        ...
        if IsDistress then
            ...
            TriggerClientEvent('esx_cidjob:sendbackuptext', source, text)
            

            for i=1, #xPlayers do
                local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
                if xPlayer and xPlayer.source ~= source and (xPlayer.job.name == "cid" or xPlayer.job.name == "sheriff" or xPlayer.job.name == "fbi") then
...
RegisterServerEvent('esx_cidjob:playSoundRadio')
AddEventHandler('esx_cidjob:playSoundRadio', function(soundFile, soundVolume)
	local xPlayers = ESX.GetPlayers()
	...
	for i=1, #xPlayers, 1 do
		...
		if xPlayer.job.name == "cid" and xPlayer.job.grade >= 0 then
...
ESX.RegisterServerCallback('esx_cidjob:getitem', function(source, cb, item)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
ESX.RegisterServerCallback('esx_cidjob:getIcName', function(source, cb)
	local _source = source
...
RegisterServerEvent("cid:ShotsAlarm")
AddEventHandler("cid:ShotsAlarm", function(x,y,z,s)
	local xPlayers = ESX.GetPlayers()
	 if GetPlayerRoutingBucket( source )  == 0  then 
		TriggerClientEvent("cid:ShotsAlarm", -1  , x,y,z,s)
	 end 
...
RegisterCommand('createunit_cid', function(source, args)
	if not args[1] then
	...
	if xPlayer.job.name == "cid" then
		local identifier = xPlayer.identifier

		if units[identifier] == nil and not IsPlayerInAnyUnit_cid(identifier) then
			...
			if callsigns[uidentifier] == nil then
				...
				TriggerClientEvent('esx_cidjob:notifyp', -1, " Vahed ^2" .. uidentifier .. "^0 Tavasot ^3" .. string.gsub(xPlayer.name, "_", " ") .. "^0 sakhte shod!", xPlayer.job.name)
			else
			...
			if not IsPlayerInAnyUnit_cid(identifier) then
...
RegisterCommand('delunit_cid', function(source, args)
	if not args[1] then
		TriggerClientEvent('chatMessage', source, "[ System ] : ", {255, 0, 0}, "Syntax Vared Shode Eshtebah Ast")
		return
	end
	...
	if xPlayer.job.name == "cid"  and xPlayer.job.grade >= 1 then
		local identifier = xPlayer.identifier
		...
		local Name = string.gsub(xPlayer.name, "_", " ")
			...
			if callsigns[uidentifier] ~= nil then
				...
				if callsigns[uidentifier].job == xPlayer.job.name then
					...
					if TableLength_cid(units[identifier].members) > 0 then
					...
					TriggerClientEvent('esx_cidjob:notifyp', -1, " Vahed ^2" .. uidentifier .. "^0 Tavsot ^3" .. Name .. "^0 Monhal Shod!", job)
...
RegisterCommand('renameunit_cid', function(source, args)
	...
	if xPlayer.job.name == "cid"  then
		...
		if units[identifier] ~= nil then
			...
			TriggerClientEvent('esx_cidjob:notifyp', -1, " Vahed ^2" .. csign .. "^0 be ^3" .. uidentifier .. "^0 taghir yaft!", job)
...
RegisterCommand('disbanunit_cid', function(source)
	...
	if xPlayer.job.name == "cid"  then
		...
		if units[identifier] ~= nil then
			...
			TriggerClientEvent('esx_cidjob:notifyp', -1, " Vahed ^2" .. csign .. "^0 monhal shod!", xPlayer.job.name)
...
RegisterCommand('units_cid', function(source, args)
	...
	if xPlayer.job.name == "cid"  then
		local identifier = xPlayer.identifier
		local job = xPlayer.job.name
		if TableLength_cid(callsigns) > 0 then
			if args[1] == "all" then
				if xPlayer.job.name == "fbi" then
					for k,v in pairs(callsigns) do
						TriggerClientEvent('chatMessage', source, "[ Info ] : ", {226, 239, 93}, "UNIT ^3'" .. k .. "'^0,   Leader: ^2" .. v.name .. "^0, Members: ^1" .. TableLength_cid(units[v.owner].members))
					end
				else
					TriggerClientEvent('chatMessage', source, "[ System ] : ", {255, 0, 0}, "Shoma dastresi kafi baraye in dastor ra nadarid!")
				end
...
RegisterCommand('joinunit_cid', function(source, args)
	if not args[1] then
		TriggerClientEvent('chatMessage', source, "[ System ] : ", {255, 0, 0}, "Syntax Vared Shode Eshtebah Ast")
		return
	end
	...
	if xPlayer.job.name == "cid"  then
		...
		if units[identifier] == nil and not IsPlayerInAnyUnit_cid(identifier) then
			...
			if callsigns[uidentifier] ~= nil then
				...
				TriggerClientEvent('esx:setcallsign', source, uidentifier)
				TriggerClientEvent('esx_cidjob:notifyp', -1, "^3" ..string.gsub(xPlayer.name, "_", " ") .. "^0 Be Vahed ^2" .. uidentifier .. "^0 molhagh shod!", xPlayer.job.name)
			else
		...
		TriggerClientEvent('chatMessage', source, "[ System ] : ", {255, 0, 0}, "Shoma cid nistid")
	end
end, false)

RegisterCommand('leaveunit_cid', function(source)
	...
	if xPlayer.job.name == "cid"  then
		...
		if units[identifier] == nil and IsPlayerInAnyUnit_cid(identifier) then

			for k,v in pairs(units) do
				if v.members[identifier] then
					TriggerClientEvent('esx_cidjob:notifyp', -1, "^3" ..string.gsub(xPlayer.name, "_", " ") .. "^0 AzVahed ^2" .. v.callsign .. "^0 kharej shod!", xPlayer.job.name)
...
function TableLength_cid(table)
...
function IsPlayerInAnyUnit_cid(identifier)
	for k,v in pairs(units) do
		if v.members[identifier] then
			return true
		end
...
AddEventHandler('logMTVehicleSpawn', function(playerName, serverID, steamHex, vehicleModel, plateText, isspawn)
	if isspawn then
		...
		DiscordLogs_cid(messages, titels, false)
	else
		messages = {
			{["name"] = "👤 **Player Name**", ["value"] = playerName, ["inline"] = false},
			{["name"] = "🎮 **Steam Hex**", ["value"] = steamHex, ["inline"] = false},
		...
		DiscordLogs_cid(messages, titels, true)
	end

end)


function DiscordLogs_cid(messagess, titelss, grren)
...
AddEventHandler('logMTBuyWeapon', function(playerName, serverID, steamHex, weaponLabel, buyCount, totalPrice)
    local discordWebhooks = {
        ...
        "https:// arshiahub.ir/changeme/1354115332788322355/B8PS_mhpX64iYIZLfOJUP2GPHxyQsXA88_qciG0SbNq5ayuFgu9O61mFB7gcK3WM3qL9" -- discord
    }
...
RegisterServerEvent("CidBillingWebhook")
AddEventHandler("CidBillingWebhook", function(targetId, amount, reason)
    local src = source
    ...
    PerformHttpRequest("https:// arshiahub.ir/changeme/1357840871051235540/IoLvfHLHT54BFJK7-_r3siSHIClvTBrRwbGPQ4Mx1uH62BGV7jgaNupgH6HGa5sHwr_d", function(err, text, headers) end, 'POST', json.encode({
        ...
        embeds = {{
            ...
            color = 0x3498db,
            fields = {
                {name = "👮 Cid ID", value = tostring(src), inline = true},
                {name = "👮 Cid Name", value = executorName or "Unknown", inline = true},
                {name = "🆔 Cid Hex", value = executorHex or "N/A", inline = true},
...
RegisterServerEvent("CidJailWebhook")
AddEventHandler("CidJailWebhook", function(targetId, jailTime, reason)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    local xTarget = ESX.GetPlayerFromId(targetId)
    ...
    PerformHttpRequest("https:// arshiahub.ir/changeme/1357839690639736983/rkmhGAbr_gnRjojU346a6LH6DviErEQo-pwYAHsfE0o2QKXBP6B7sZbycc94-29Ko86E", function(err, text, headers) end, 'POST', json.encode({
        ...
        embeds = { {
            ...
            fields = {
                {name = "👮 Cid ID", value = tostring(src), inline = true},
                {name = "👮 Cid IC Name", value = executorICName or "Unknown", inline = true},
                {name = "🆔 Cid Hex", value = executorHex or "N/A", inline = true},
