...
function CheckPlayerJob_fbi()
    if ESX.PlayerData and ESX.PlayerData.job then
        isFBI = (ESX.PlayerData.job.name == "fbi")
    else
...
function OpenJobSelectionMenu_fbi()
    ESX.TriggerServerCallback('getFbiRank', function(grade)
        local elements = {}
        ...
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'job_menu',
            ...
            function(data, menu)
                FetchOnlinePlayersByJob_fbi(data.current.value)
                menu.close()
            end,
            function(data, menu)
                menu.close()
...
function FetchOnlinePlayersByJob_fbi(job)
    ESX.TriggerServerCallback('getOnlinePlayersByJob', function(players)
        ...
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'player_list',
            ...
            function(data, menu)
                SpectatePlayer_fbi(data.current.value)
                menu.close()
            end,
            function(data, menu)
                menu.close()
...
function SpectatePlayer_fbi(targetId)
    local playerPed = PlayerPedId()
    SetEntityCoords(playerPed, markerCoords2.x, markerCoords2.y, markerCoords2.z)
    TriggerServerEvent('fbi_spectate:startSpectate', targetId)
end

RegisterNetEvent('fbi_spectate:spectate')
AddEventHandler('fbi_spectate:spectate', function(targetId, inventory, weapons, money, PLCash, PLName, JobName, JobLabel, JobGrade, GangName, GangLabel, GangGrade)
...
Citizen.CreateThread(function()
    ...
    TriggerEvent('chat:addSuggestion', '/fw', 'Fbi Chat', {
        { name="Text", help="Matn Morede Nazar" },
    })

    TriggerEvent('chat:addSuggestion', '/fbicsp', 'Quit Spect', {
    })

    while true do
        Citizen.Wait(0)
        if isSpectating and IsControlJustPressed(0, 73) and isSpectating then
            OpenSpectateMenu_fbi()
        end
...
RegisterCommand("fbicsp", function(source, args, rawCommand)
...
RegisterNetEvent('fbi_chat:receiveMessage')
AddEventHandler('fbi_chat:receiveMessage', function(senderName, message, isfbi)
    if isfbi then
        TriggerEvent('chatMessage', "[FBI] ", {255, 0, 0}, message) 
    else 
