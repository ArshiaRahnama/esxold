...
function Sentence(type, time, part)
	...
	elseif type == "jail" then
		...
		sentence.coords = playercoords
		sentence.distance = 5
		ESX.Game.Teleport(ped, playercoords)
		SetEntityHeading(ped, playercoords)
...
function UnJail()
    ...
    local place = LocationNazdik(pcoords)

    if not place then
        ESX.ShowNotification("~r~Makanie Baraye Azadi Peida Nashod!")
        return
    end
    ...
    ESX.Game.Teleport(ped, {x = place.x, y = place.y, z = place.z})
    SetEntityHeading(ped, place.heading or 0.0)
...
-- function LocationNazdik(pcoords)
--     if not pcoords or not pcoords.x or not pcoords.y or not pcoords.z then
--         print("^1[ERROR] LocationNazdik: pcoords نامعتبر است!^7")
--         return nil
--     end
...
-- 	for _, data in pairs(Config.Unjails2) do
-- 		print(data.x, data.y, data.z)
-- 		local distance = GetDistanceBetweenCoords(pcoords.x, pcoords.y, pcoords.z, data.x, data.y, data.z, true)
-- 		if distance < closestDistance then
-- 			print('ss')
-- 			closestLocation = data
-- 		end
-- 	end
...
function LocationNazdik(pcoords)
    -- بررسی مقدار pcoords
    if not pcoords or type(pcoords) ~= "vector3" then
        print("^1[ERROR] pcoords نامعتبر است یا مقدار ندارد!^7")
        return nil
    end

    local closestLocation = nil
    local closestDistance = math.huge  -- مقدار بزرگ برای مقایسه اولیه

    -- بررسی اینکه Config.Unjails2 مقدار دارد
    if Config.Unjails2 and type(Config.Unjails2) == "table" then
        for key, data in pairs(Config.Unjails2) do
            -- بررسی مقدار موقعیت‌های Config.Unjails2
            if type(data) ~= "table" or not data.x or not data.y or not data.z then
                print("^1[ERROR] مختصات نامعتبر در Config.Unjails2 در key: " .. tostring(key) .. "^7")
                goto continue  -- ادامه حلقه اگر مقدار نامعتبر بود
            end

            -- محاسبه فاصله
            local distance = #(pcoords - vector3(data.x, data.y, data.z))
            if distance < closestDistance then
        ...
        print("^1[ERROR] Config.Unjails2 مقدار ندارد یا جدول نیست!^7")
    end
...
local rrr = LocationNazdik(GetEntityCoords(GetPlayerPed(source)))
print(rrr)
