...
function Sentece(source, target, type, time, reason, part, notif)
	local identifier = GetPlayerIdentifier(target)
	JobStuff(target)
	AdminStuff(target)
...
AddEventHandler("OffDutyHandlerForJail",function()
		...
		TriggerClientEvent("aduty:tagChanger", false)
		AdminPerks = false
		ShowID = false
...
function AdminStuff(target)
	...
	if xPlayer then
	  ...
	  if xPlayer.get('aduty') then
		exports.esx_aduty:DutyHandlerForJail(target, false)
		--TriggerEvent('OffDutyHandlerForJail', target, true)
	  end
	end
end
