...
function OpenAccessoryMenu()
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'set_unset_accessory',
	{
		...
		elements = {
			...
			{label = _U('remove_vest'), value = 'Vest'}
		}
	}, function(data, menu)
		menu.close()
...
function SetUnsetAccessory(accessory)
	...
	if accessory == "Vest" then
		...
		if Armor > 0 then	
			...
			ESX.ShowNotification('Shoma Vest nadarid!')
...
Citizen.CreateThread(function()
	...
	while true do
		...
		for k,v in pairs(Config.Zones) do
			for i = 1, #v.Pos, 1 do
				distance = #(coords - vector3(v.Pos[i].x, v.Pos[i].y, v.Pos[i].z))
				if(Config.Type ~= -1 and distance < 20.0) then
