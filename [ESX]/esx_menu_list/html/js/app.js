(function(){
	...
	let MenuTpl =
		'<div id="menu_{{_namespace}}_{{_name}}" class="menu" >' +
			'<div class="menu-search-wrap">' +
				'<input type="text" class="menu-search" placeholder="Search" oninput="ESX_MENU.filter(this)" />' +
			'</div>' +
			'<div class="menu-grid">' +
				'{{#rows}}' +
					'<div class="menu-card">' +
						'{{#cols}}<div class="menu-cell">{{{content}}}</div>{{/cols}}' +
					'</div>' +
				'{{/rows}}' +
			'</div>' +
		'</div>'
	...
	ESX_MENU.filter = function(input){
		let term  = input.value.toLowerCase();
		let cards = $(input).closest('.menu').find('.menu-card');
