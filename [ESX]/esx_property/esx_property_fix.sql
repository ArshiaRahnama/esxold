-- ============================================================================
-- esx_property FIX MIGRATION (نسخه‌ی نهایی با ۴۲ خونه‌ی واقعی)
-- ============================================================================
-- این فایل جدول `properties` رو با ستون‌های صحیحی که server/main.lua
-- اسکریپت esx_property نیاز داره (entering, exit, inside, outside, ipls,
-- gateway, is_single, is_room, is_gateway, room_menu, storage_data)
-- می‌سازه و ۴۲ خونه/آپارتمان واقعی (که مختصات و اسم‌هاشون از دیتاست
-- khohne.sql گرفته شده) رو توش درج می‌کنه.
--
-- تغییراتی که نسبت به khohne.sql دادم:
--   1) ستون `storage_data` اضافه شد (کد سرور بهش نیاز داره؛ برای همه
--      ردیف‌ها مقدار 'property' گذاشته شده).
--   2) باگ JSON نامعتبر تو ردیف NorthConkerAvenue2045 (فیلد exit یه
--      کاما اضافه‌ی داخل رشته داشت که باعث خطای json.decode می‌شد) رفع شد.
--   3) متن خراب/mojibake شش تا از لیبل‌های فرانسوی (Séduisant x3،
--      Royal x3) که encoding‌شون خراب شده بود، تمیز شد.
--   4) `CREATE DATABASE` / `USE xcoorerpsv2` که تو فایل اصلی بود حذف شد
--      چون قراره این جدول تو دیتابیس خودت (همون essentialmode) ساخته بشه.
--
-- نحوه‌ی اجرا: این فایل رو با phpMyAdmin/HeidiSQL/mysql CLI روی همون
-- دیتابیسی که سرورت بهش وصله اجرا کن، بعد resource رو ری‌استارت کن.
-- ============================================================================
...
-- Dumping data for table `properties`: 42 rows
INSERT INTO `properties` (`id`, `name`, `label`, `entering`, `exit`, `inside`, `outside`, `ipls`, `gateway`, `is_single`, `is_room`, `is_gateway`, `room_menu`, `price`, `storage_data`) VALUES
