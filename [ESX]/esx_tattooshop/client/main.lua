...
function RemoveTattoo(name, label)
	...
	ESX.ShowNotification("Shoma Tattoo ~y~" .. GetLabelText(label) .. "~s~ Ra ~r~Remove~s~ Kardid!")
end
    ...
    while true do 
		...
		for k, v in ipairs(Config.TattooCats) do
			if JayMenu.IsMenuOpened(v[1]) then
				...
				for _, tattoo in pairs(json.decode(LoadResourceFile(GetCurrentResourceName(), 'AllTattoos.json'))) do
