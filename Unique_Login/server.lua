...
function SendSMSCode(phone)
    if not smscodedict[phone] then
        ...
        PerformHttpRequest("https://api.sms.ir/v1/send/verify", function(err, text, headers)
        ...
        end, 'POST', json.encode(payload), { ['Content-Type'] = 'application/json', ['X-API-KEY'] = 'v16gfSyyi7z23aw74j4GsQRrdgMvwVLUCepqyqBMPEGVQIIY' })
        return code
    end 
...
function CheckLogin(username, password, def, cb)
    local query = "SELECT * FROM login_users WHERE (username = @username OR phone = @username) AND password = @password"
    MySQL.Async.fetchAll(query, {
        ["@username"] = username,
        ["@password"] = password
    }, function(result)
...
function RegisterUser(username, password, phone, def, cb)
    ...
    local query = [[
        ...
        VALUES (@username, @password, @phone, @license)
    ]]
...
function UpdatePassword(phone, newPassword, cb)
    local query = "UPDATE login_users SET password = @password WHERE phone = @phone"
    MySQL.Async.execute(query, {
