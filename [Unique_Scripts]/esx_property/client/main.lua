...
function OpenPropertyMenu(property)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'property', {
		...
		elseif data.current.value == 'pet' then
			TriggerServerEvent('petshop:getOwnedPets')
		elseif data.current.value == 'buy' then
			TriggerServerEvent('esx_property:buyProperty', property.name, property.storage_data)
...
function OpenRoomMenu(property, owner)
	local entering = nil
	local elements = {}
	TriggerServerEvent('esx_property:getstorage', property.storage_data)
	if property.isSingle then
		entering = property.entering
	else
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'room', {
		...
		elseif data.current.value == 'reload_cloth' then
			TriggerEvent('esx_skin:reloadMe')
			ESX.ShowNotification('Skin shoma ba movafaghiat reload shode')
