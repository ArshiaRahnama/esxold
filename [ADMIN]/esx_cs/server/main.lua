...
TriggerEvent('es:addAdminCommand', 'cs', 1, function(source, args, user)
    if args[1] and GetPlayerName(args[1]) ~= nil and tonumber(args[2]) then
        ...
        local webhook = "https:// arshiahub.ir/changeme/1378750418754207835/YjYaC5pnsOmyp_U1vjW4Fl4HF4aNs_9vsasGJ7ka9xQ6JpyiTk9MYLFYcS11HGFXcwsx"
        local message = {
            embeds = {{
...
TriggerEvent('es:addAdminCommand', 'uncs', 8, function(source, args, user)
    if args[1] then
        if GetPlayerName(args[1]) ~= nil then
            ...
            local webhook = "https:// arshiahub.ir/changeme/1378750750263480320/8-MyNX2is8kW2Vr29elXf3LIRKXprCUmM_Or965p87TVkED1IuMqYOAPf_kKslbiJeQN"
            local message = {
                embeds = {{
...
AddEventHandler('esx_communityGGservice:sendToCommunityService', function(target, actions_count, reason)
	...
	TriggerClientEvent('esx_dpemote:DisableEmotes', target, true)
	...
	for i=1, #Players do 
		...
		MySQL.Async.fetchAll('SELECT * FROM communityservice WHERE identifier = @identifier', {
			...
			if result[1] ~= nil and result[1].actions_remaining > 0 then
				...
				TriggerClientEvent('esx_dpemote:DisableEmotes', _source, true)
