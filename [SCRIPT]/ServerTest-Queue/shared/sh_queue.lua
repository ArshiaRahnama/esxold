...
function Queue:HasTempPriority(ids)
    ...
    for _, id in pairs(ids) do
        ...
        if tmpPrio[id] then return tmpPrio[id].power, tmpPrio[id].ServerTest, id end

        if string_sub(id, 1, 5) == "steam" then
            local steamid = Queue:HexIdToSteamId(id)
            if tmpPrio[steamid] then return tmpPrio[steamid].power, tmpPrio[steamid].ServerTest, id end
        end
    end
...
function Queue:AddToQueue(ids, connectTime, name, src, deferrals)
    ...
    for pos, data in ipairs(queueList) do
        if tmp.priority then
            ...
            if _pos then
                Queue:DebugPrint(string_format("%s[%s] Vared Saf Shodid %d/%d Dar Saf", tmp.name, ids[1], _pos, queueCount))
                break
            end
        end
    end
    ...
    if not _pos then
        ...
        Queue:DebugPrint(string_format("%s[%s] Mogheiat Shoma %d/%d Dar Saf", tmp.name, ids[1], _pos, queueCount))
    end
...
function Queue:CanJoin(src, cb)
    ...
    for _, data in ipairs(_Queue.JoinCbs) do
        ...
        while await do Citizen.Wait(1) end
...
local function playerConnect(name, setKickReason, deferrals)
    ...
    if Queue:IsInQueue(ids) then
        ...
        Queue:DebugPrint(string_format("%s[%s] Bad Az Cancel Kardan Be Saf Bargasht", name, ids[1]))
    else
        Queue:AddToQueue(ids, connectTime, name, src, deferrals)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        for id, data in pairs(Queue:GetTempPriorityList()) do
            if os_time() >= data.ServerTest then
...
if Config.DisableHardCap then
    Queue:DebugPrint("^1 [ServerTest-Queue] Disabling hardcap ^7")
...
commands.addq = function()
    ...
    Queue:AddToQueue({"steam:110000146d830cd"..testAdds}, os_time(), "TestAdd: " .. testAdds, "debug")
    testAdds = testAdds + 1
end
...
commands.printtp = function()
    ...
    for k, data in pairs(Queue:GetTempPriorityList()) do
        Queue:DebugPrint(k .. ": Power: " .. tostring(data.power) .. " | ServerTest: " .. tostring(data.ServerTest) .. " | CurTime: " .. tostring(os_time()))
    end
end
