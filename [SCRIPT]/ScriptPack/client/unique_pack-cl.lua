...
-- ====================================================================
ESX = nil
local lastBadgeUse = 0 -- متغیر برای نگهداری زمان آخرین استفاده از کامند
...
RegisterCommand('showbadge', function()
    ...
    -- چک کردن اینکه آیا 10 ثانیه از آخرین استفاده گذشته است
    if (currentTime - lastBadgeUse) < cooldownTime then
        lib.notify({ position = 'center-right', title = '', description = 'Spam Nakonid!', type = 'error', duration = 5000 })
        return
    end
    ...
    local playerJobGrade = ESX.GetPlayerData().job.grade_label
    local playerName = GetPlayerName(playerId)

    -- چک کردن اگر بازیکن سوار ماشین است یا اسلحه در دست دارد
    if IsPedInAnyVehicle(playerPed, false) then
        lib.notify({ position = 'center-right', title = '', description = 'شما نمی‌توانید در حالی که سوار ماشین هستید از این فرمان استفاده کنید!', type = 'error', duration = 5000 })
        return
    end

    if IsPedArmed(playerPed, 7) then
        lib.notify({ position = 'center-right', title = '', description = 'شما نمی‌توانید در حالی که اسلحه در دست دارید از این فرمان استفاده کنید!', type = 'error', duration = 5000 })
        return
    end
    ...
    if isJobAllowed(playerJob, allowedJobs) then
        ...
        lastBadgeUse = currentTime -- به‌روزرسانی زمان آخرین استفاده

        -- اجرای انیمیشن و نمایش پروپ badge
        ...
        lib.notify({ position = 'center-right', title = '', description = 'شما اجازه استفاده از این فرمان را ندارید!', type = 'error', duration = 5000 })
    end
end, false)

-- تابع برای چک کردن اینکه آیا شغل مجاز است
