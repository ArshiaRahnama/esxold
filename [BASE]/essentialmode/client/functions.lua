    ...
    menu.close = function()
        ...
        for i = 1, #ESX.UI.Menu.Opened, 1 do
            if ESX.UI.Menu.Opened[i] ~= nil then
                if
                    ESX.UI.Menu.Opened[i].type == type and ESX.UI.Menu.Opened[i].namespace == namespace and
                        ESX.UI.Menu.Opened[i].name == name
                 then
...
ESX.Game.DeleteVehicle = function(vehicle)
	TriggerServerEvent("CarLock:ToggleKey", false, GetVehicleNumberPlateText(vehicle), vehicle)
...
ESX.Game.SpawnVehicle = function(modelName, coords, heading, cb, plate)
    ...
    Citizen.CreateThread(function()
        ...
        local vehicleLabel = GetLabelText(GetDisplayNameFromVehicleModel(model))

        if plate then
            local pt = ESX.Math.Trim(plate)
            SetVehicleNumberPlateText(vehicle, string.upper(pt))
            TriggerServerEvent('esx:CreateItem', "CarKey|"..string.upper(pt), vehicleLabel.." | "..string.upper(pt), 1, 0, 0)
            TriggerServerEvent("CarLock:ToggleKey", true, string.upper(pt), vehicle)
            TriggerServerEvent('registerSpawnedVehicle', GetVehicleNumberPlateText(vehicle))
        else
            TriggerServerEvent('registerSpawnedVehicle', GetVehicleNumberPlateText(vehicle))
            TriggerServerEvent('esx:CreateItem', "CarKey|"..GetVehicleNumberPlateText(vehicle), vehicleLabel.." | "..GetVehicleNumberPlateText(vehicle), 1, 0, 0)
...
ESX.Game.GetClosestObject = function(filter, coords)
    ...
    for i = 1, #objects, 1 do
        ...
        if foundObject then
            ...
            local targetx = vector3(coords.x, coords.y, coords.z)
            local distance = #(objectCoords - targetx)
...
ESX.Game.GetClosestPlayer = function(coords)
    ...
    for i = 1, #players, 1 do
        ...
        if not usePlayerPed or (usePlayerPed and players[i] ~= playerId) then
            local targetCoords = GetEntityCoords(target)
			local targetx = vector3(coords.x, coords.y, coords.z)
            local distance = #(targetCoords - targetx)
...
ESX.Game.GetClosestVehicle = function(coords)
    ...
    for i = 1, #vehicles, 1 do
		...
		local targetx = vector3(coords.x, coords.y, coords.z)
        local distance = #(vehicleCoords - targetx)
...
ESX.Game.GetVehicleInDirection = function(distance)
    ...
    local RayHandle =
        StartShapeTestRay(camCoords.x, camCoords.y, camCoords.z, farCoordsX, farCoordsY, farCoordsZ, -1, playerPed, 0)
...
ESX.Game.GetClosestPed = function(coords, ignoreList)
    ...
    for i = 1, #peds, 1 do
		...
		local targetx = vector3(coords.x, coords.y, coords.z)
        local distance = #(pedCoords - targetx)
...
ESX.Game.Utils.DrawText3D = function(coords, text, size, r, g, b)
    local onScreen, x, y = World3dToScreen2d(coords.x, coords.y, coords.z)
    local camCoord = GetGameplayCamCoord()
	local targetx = vector3(coords.x, coords.y, coords.z)
    local dist = #(camCoord - targetx)
    local size = size
...
ESX.DoesHaveItem = function(name, count, cb, label)
    ...
    ESX.ShowNotification("Shoma hade aqal be " .. count .. " adad " .. ESX.FirstToUpper(tname) .. " Niyaz darid!")
end
                            ...
                            function(data2, menu2)
                                ...
                                if type == "item_weapon" then
                                    ...
                                    TriggerServerEvent(
                                        ...
                                        GetPlayerServerId(nearbyPlayer),
                                        type,
                                        item,
                                        sourceAmmo
                                    )
...
AddEventHandler(
    ...
    function(title, subject, msg, icon, iconType)
