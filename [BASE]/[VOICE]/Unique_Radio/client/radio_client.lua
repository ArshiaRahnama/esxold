...
Radio.Commands = {
	...
	},
	{
		...
		Params = {
			{ name = "number", help = "Enter frequency" }
		},
		Handler = function(src, args, raw)
			if Radio.Has then
				if args[1] then
...
AddEventHandler('esx:setJob', function(job)
	if not PlayerData or not job then return end

	if not PlayerData.job or job.name ~= PlayerData.job.name then
		if PlayerData.job and SI.PrivateFrequency[PlayerData.job.name:lower()] then
			for k,v in pairs(SI.PrivateFrequency[PlayerData.job.name:lower()]) do
...
function Radio:Add(id, silent)
	...
	if id ~= 0 and id ~= 40 and id ~= 41 then
		ExecuteCommand("me radio ro roshan mikone va vared mishe.")
	end
end
		...
		if Radio.Open then
			...
			if IsControlJustPressed(0, radioConfig.Controls.Toggle.Key) then
				...
				if Radio.On then
					...
					Radio:Remove()
					ExecuteCommand("me radio khodesho khamoosh mikone.")
				end
			end
