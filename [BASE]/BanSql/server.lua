...
TriggerEvent('es:addAdminCommand', 'banreload', Config.permission, function (source)
	loadBanList()
...
RegisterCommand('unban', function(source, args)
	...
	if source == 0 or xPlayer.permission_level >= Config.permission then
		if args[1] then
			local identifier = args[1]
			
				MySQL.Async.fetchAll('DELETE FROM banlist WHERE identifier = @identifier', 
				{
					['@identifier'] = identifier
				}, function(data)
					TriggerEvent('bansql:sendMessage', source, '^4[' .. identifier .. ']' .. ' ^1 Has Been ^*Unbaned!')
					loadBanList()
					TriggerEvent('DiscordBot:ToDiscord', 'bansystem', 'BanSystem', identifier .. " Tavasot " .. GetPlayerName(source) .. " (" .. xPlayer.name .. ") unban shod" ,'user', true, source, false)
				end)
...
RegisterCommand('ban', function(source, args)
	...
	if xPlayer.permission_level >= Config.permission then
		...
		if args[1] then		
			if reason == "" then
				reason = 'Shoma Ban Shodid, Baraye Etela\'ate Bishtar Be discord.gg/DiscordLink moraje konid!'
			end
			if target and target > 0 then
...
RegisterCommand('banoffline', function(source, args)
	...
	if xPlayer.permission_level >= Config.permission then
		if not args[1] then
			TriggerClientEvent('chatMessage', source, "[SYSTEM]", {255, 0, 0}, "Shoma dar ghesmat Steamhex chizi vared nakardid")
			return
		end
		if tonumber(args[1]) then
			TriggerClientEvent('chatMessage', source, "[SYSTEM]", {255, 0, 0}, "Shoma dar ghesmat Steamhex nemitavanid ID vared konid")
			return
		end
		...
		MySQL.Async.fetchAll("SELECT * FROM baninfo WHERE LOWER(identifier) = @identifier", 
		{
			['@identifier'] = string.lower(target)
		}, function(data)
			if data[1] then
				...
				if duration > 0 then
					...
					TriggerClientEvent('chatMessage', -1, "[Ban]", {255, 0, 0}, "^1" .. data[1].oocname .. " ^0tavasot ^2" .. sourceplayername .. " ^0ban shod be modat ^8" .. duration .. " ^0rooz be dalile: ^3" .. reason )
					...
					TriggerClientEvent('chatMessage', -1, "[Ban]", {255, 0, 0}, "^1" .. data[1].oocname .. " ^0tavasot ^2" .. sourceplayername .. " ^0permanent ban shod be dalile: ^3" .. reason )
...
function SteamCheck(identifier)
	for i=1, #BanList do
		if string.lower(BanList[i].identifier) == identifier then 
			table.remove(BanList, i)
			return true
		end 
	end
