...
function onSubmit() {
    lobbyname = $('#lname');
    lobbypass = $('#lbpass');
    friendlyFire = false; // no UI control exists for this yet (was reading a non-existent #fire element, always undefined)
    roundNum = $('#round');
    timer = $('#timer');
...
function onStart() {
    ...
    fetch(`https://${ResourceName}/StartMatch`, {
        ...
        if (data && data.ok === false) {
            popupkon('Cannot start', data.reason || 'Match could not be started.');
        }
    });
};
...
function teamClass(team) {
    var t = String(team);
    if (t == "1" || t.toLowerCase() == "blue" || t.toLowerCase() == "teamone") return "team-blue";
    if (t == "2" || t.toLowerCase() == "red" || t.toLowerCase() == "teamtwo") return "team-red";
    return "team-neutral";
}

var carIcon = '<svg class="kf-icon" viewBox="0 0 24 24" fill="currentColor"><path d="M18.9 6.3c-.2-.6-.8-1-1.4-1H6.5c-.6 0-1.2.4-1.4 1L3.5 11v7c0 .6.4 1 1 1h1c.6 0 1-.4 1-1v-1h11v1c0 .6.4 1 1 1h1c.6 0 1-.4 1-1v-7l-1.6-4.7zM6.8 7h10.4l1 3H5.8l1-3zM7 15.5c-.8 0-1.5-.7-1.5-1.5S6.2 12.5 7 12.5s1.5.7 1.5 1.5S7.8 15.5 7 15.5zm10 0c-.8 0-1.5-.7-1.5-1.5s.7-1.5 1.5-1.5 1.5.7 1.5 1.5-.7 1.5-1.5 1.5z"/></svg>';
var headshotIcon = '<svg class="kf-icon kf-headshot" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="9"/><path d="M12 3v4M12 17v4M3 12h4M17 12h4"/></svg>';
...
function addKill(killer, weapon, killed, headshot) {
    ...
    let killerClass = teamClass(killer.team);
    let killedClass = teamClass(killed.team);

    let weaponHtml = (weapon == "VEHICLE")
        ? carIcon
        : `<img class="kf-weapon" src="./assets/weapons/${weapon}.png" onerror="this.style.display='none'">`;

    let killfeedElement = `
        <div class="kill-wrapper">
            <div class="kill">
                <p class="${killerClass}">${killerName}</p>
                ${weaponHtml}
                ${(headshot) ? headshotIcon : ""}
                <p class="${killedClass}">${killedName}</p>
            </div>
        </div>
    `;
    ...
    setTimeout(() => { elem.hide(500); setTimeout(() => { elem.remove(); }, 500); }, 5000);
}
...
window.addEventListener("message", function(event)
{
	...
	else if(event.data.action == "AddKill")
	{
		// Expected payload:
		// { action: "AddKill", killer: {name, team}, killed: {name, team}, weapon: "pistol" | "VEHICLE", headshot: true|false }
		// weapon should match a filename in html/assets/weapons/ (without .png), or "VEHICLE" for roadkills.
		// team should be 1 (blue), 2 (red), or anything else (shown neutral).
 ...
 $(document).ready(function () {
            ...
            $('input[value=jail]').on('click', function () {
            ...
            $('input[value=shop2]').on('click', function () {
				...
				$('#javaheri-checkbox').css('display', 'none');
				$('#shop1-checkbox').css('display', 'none');
				$('#shop2-checkbox').css('display', 'block');
				$('#1v1-checkbox').css('display', 'none');
	...
	$.post('https:/paintball/back', JSON.stringify());
            })
