...
CreateThread(function()
	...
	if rows then
		for i = 1, #rows do
			PBJobAccess[rows[i].job_name] = rows[i].min_grade
		end
	end
end)

ESX.RegisterServerCallback('esx_paintball:GetJobAccessGrade', function(source, cb, jobName)
...
local function IsJobBoss(xPlayer)
	...
	return job.grade_name == 'boss' or job.grade_label == 'Boss' or job.grade_label == 'boss'
end

RegisterServerEvent('esx_paintball:setJobAccessGrade')
AddEventHandler('esx_paintball:setJobAccessGrade', function(society, grade)
	local source = source
	...
	if not grade or grade < 0 or grade ~= math.floor(grade) then
		TriggerClientEvent('esx_paintball:Notify', source, 'Invalid grade.')
		return
	end

	local jobName = xPlayer.job.name
	PBJobAccess[jobName] = grade
	MySQL.insert.await('INSERT INTO paintball_job_access (job_name, min_grade) VALUES (@job, @grade) ON DUPLICATE KEY UPDATE min_grade = @grade', {
		['@job'] = jobName,
		['@grade'] = grade
	})
...
function StartMatch(LobbyId, PlayerId)
	...
	if tempTeams[1] == 0 or tempTeams[2] == 0 then
		return false, "Each team needs at least one ready player"
	end
	if (tempTeams[1] + tempTeams[2]) ~= (#LobbyList[_].teams[2] + #LobbyList[_].teams[3]) then
...
ESX.RegisterServerCallback('esx_paintball:StartMatch', function(source, cb, data)
	local ok, reason = StartMatch(tonumber(data.LobbyId), source)
...
RegisterServerEvent('unique_paintball:inPaintBall')
AddEventHandler('unique_paintball:inPaintBall', function(state)
	local xPlayer = ESX.GetPlayerFromId(source)
	local identifier = xPlayer.identifier
	if state == true then
		InGame[identifier] = state
	else
		InGame[identifier] = nil
	end
end)
