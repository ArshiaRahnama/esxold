...
RegisterNetEvent('Admin_Menu:SpectMenus')
AddEventHandler('Admin_Menu:SpectMenus', function(Cheks)
	spect2s = Cheks
end)
...
function OpenAdminActionMenu(player)
    ...
    ESX.TriggerServerCallback('esx_spectate:getOtherPlayerData', function(data)
	  ...
	  if data.gang.grade_label ~= nil and  data.gang.grade_label ~= '' then
        GangLabel = 'Gang : ' .. data.gang.name .. ' - ' .. data.gang.grade_label.. ' | '.. data.gang.grade
      else
...
Citizen.CreateThread(function()
  	...
  	while true do
		...
		if InSpectatorMode then
			...
			if vehicle ~= 0 then
				table.insert(text,"Vehicle Engine : "..GetVehicleEngineHealth(vehicle).."/1000.0")
			end
