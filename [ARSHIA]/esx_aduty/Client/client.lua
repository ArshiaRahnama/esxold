	...
	ESX.TriggerServerCallback('esx_aduty:checkAdmin', function(isAdmin)
        if isAdmin then
			...
			if not IsPedInAnyVehicle(GetPlayerPed(Target), false) then
                ...
                -- print(SetEntityCoords(PlayerPedId(), Target.coords.x, Target.coords.y, Target.coords.z))
                -- tt = GetEntityCoords(GetPlayerFromServerId(player))
                -- SetEntityCoords(PlayerPedId(), )
                -- print(GetEntityCoords(GetPlayerFromServerId(args[1])))
				-- return TriggerEvent('chat:addMessage', {color = { 255, 0, 0}, multiline = true, args = {"[SYSTEM]", "In Player Savar Mashin Nist!"}})
...
function HDraw3DText(x,y,z,textInput,fontId,scaleX,scaleY)
    ...
    SetTextFont(fontId)
    SetTextProportional(1)
    SetTextColour(rgb.r, rgb.g, rgb.b, 255)		-- You can change the text color here
