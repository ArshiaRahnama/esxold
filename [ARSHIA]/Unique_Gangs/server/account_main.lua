...
MySQL.ready(function()
	...
	for i=1, #result, 1 do
		...
		if shared ~= 0 then
			...
			if #result2 == 0 then
				MySQL.Sync.execute('INSERT INTO gang_account_data (gang_name, money, dirty_money, owner) VALUES (@gang_name, @money, @dirty_money, NULL)',
				{
					['@gang_name'] 		= name,
					['@money']        	= 0,
					['@dirty_money']    = 0
				...
				dirty_money = result2[1].dirty_money
				pay		= result2[1].pay
			end

			local gangAccount   = CreateGangAccount(name, nil, money, dirty_money, pay)
