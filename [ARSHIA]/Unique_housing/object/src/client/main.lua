              ...
              if not furni.antiGrab[model] then
                ...
                if IsDisabledControlJustReleased(0, Controls["Grab"]) or IsDisabledControlReleased(0, Controls["Grab"]) and wasPressing then
                  if keyHeldFor >= 500 then
                    ...
                    if exports['sunset_housing']:GetObjectId(ent) then
                      EditID = exports['sunset_housing']:GetObjectId(ent)
                    end
        ...
        elseif furni.doingRemoval then
          ...
          if ent and ent ~= -1 and ent ~= 0 then
            ...
            if ePos and ePos.x and ePos.x ~= 0 and ePos.x ~= 0.0 then
              local model = GetEntityModel(ent)
              if not furni.antiGrab[model] then
                Utils.drawText3D(pos,"~y~Hold [~s~LMB/RT~y~] to delete [~s~"..(hashtable[model] and hashtable[model].name or "Unknown").."~y~] [~s~"..(hashtable[model] and "$"..math.floor(hashtable[model].price*(0.3)) or "Unknown").."~y~]~s~",1,7)
                --Utils.drawText3D(pos,"~y~Hold [~s~LMB/RT~y~] to delete [~s~"..(hashtable[model] and hashtable[model].name or "Unknown").."~y~]",1,7)
                if IsDisabledControlPressed(0, Controls["Grab"]) or IsControlPressed(0, Controls["Grab"]) and not furni.focused then
                  keyHeldFor = keyHeldFor + (GetGameTimer() - lastFrame)
                  if keyHeldFor >= 500 then
                    keyHeldFor = 0
...
furni.remove = function(ent,data)
  ...
  local id = exports['sunset_housing']:GetObjectId(ent)
  furni.controlling = false
  TriggerServerEvent('sunset_housing:DeleteFurniture',furni.inHouse,id)
end
...
furni.doPlace = function(...)
  ...
  if furni.inHouse then
    ...
    local pos = GetEntityCoords(furni.object) - GetEntityCoords(furni.shell)
    local rot = GetEntityRotation(furni.object, 2)
    ...
    if furni.wasEditing then
      ...
      DeleteEntity(cobject)
      TriggerServerEvent('sunset_housing:ReplaceFurniture', furni.inHouse,data)
    else
      ESX.TriggerServerCallback('sunset_housing:CanBuy', function(canAfford)
        if canAfford then
          local data = {
            item = item,
            pos = pos,
          ...
          DeleteEntity(cobject)
          TriggerServerEvent('sunset_housing:PlaceObject', furni.inHouse,data)
        else
...
Citizen.CreateThread(function()
  ...
  --     print(v2.object)
  --     ESX.Game.SpawnLocalObject(v2.object,GetEntityCoords(PlayerPedId()),function(obj)
  --       -- print(obj)
  --       -- furni.object = obj
...
furni.doSelect = function(obj) 
  ...
  ESX.Game.SpawnLocalObject(hash,vector3(furni.hitPos.x,furni.hitPos.y,furni.hitPos.z),function(obj)
    furni.object = obj
    furni.spawnedObjects[#furni.spawnedObjects+1] = furni.object
    Wait(10)
...
-- PLAYERHOUSING STUFF
furni.enterHouse = function(house,shell)
  furni.inHouse = house
  furni.shell = shell
  furni.display(true)
  furni.focus(false)
