...
local leaningAnimationsets = {
    ...
    RegisterKeyMapping('_SynSit_Lean', 'Lean Anywhere', 'keyboard',  'l')
    RegisterCommand('_SynSit_Lean', function()
        if not isCurrentlyLeaning then
                ...
                if detected and coords then
                    if not SkipSeat then
                        ...
                        repeat
                            ...
                            animSet = leaningAnimationsets[randomAnim].animations
                            Wait(10)
                            refetch = PlayerGender == 'male' and leaningAnimationsets[randomAnim].gender == 'female'
                        ...
                        TaskPlayAnim(playerPed, currentDict, entryAnim, 8.0, -8.0, GetAnimDuration(currentDict, entryAnim), 0, 0, false, false, false)
                        Citizen.Wait(GetAnimDuration(currentDict, entryAnim) * 1000)


                        local currentDict = animSet.dict .. animSet.idle.suffix
                        loadAnimDict(currentDict)
                        local randomAnim_idle = math.random(1,#animSet.idle.anims)
                        TaskPlayAnim(playerPed, currentDict, animSet.idle.anims[randomAnim_idle], 8.0, -8.0, -1, 1, 0, false, false, false)
                        Citizen.Wait(100)
                        local exitSide = "front"
                        while IsEntityPlayingAnim(playerPed, currentDict, animSet.idle.anims[randomAnim_idle], 3) do
                            Citizen.Wait(5)
                        ...
                        TaskPlayAnim(playerPed, currentDict, exitAnim, 8.0, -8.0, GetAnimDuration(currentDict, exitAnim), 0, 0, false, false, false)
                        Citizen.Wait(GetAnimDuration(currentDict, exitAnim) * 1000)
    ...
    function DetectWall(pos)
        ...
        for _, direction in ipairs(directions) do
            local checkPos = GetOffsetFromEntityInWorldCoords(PlayerPedId(),direction.x,direction.y,direction.z)
            local rayHandle = StartShapeTestRay(pos.x, pos.y, (pos.z), checkPos.x, checkPos.y, checkPos.z, -1 , PlayerPedId(), 0)
