...
$(document).on('click', '.phone-app-footer-button', function(e){
    ...
    if (PressedFooterTab !== CurrentFooterTab) {
        ...
        if (PressedFooterTab == "recent") {
            $.post('http://Unique_Phone/ClearRecentAlerts');
...
$(document).on('click', '.phone-recent-call', function(e){
    ...
    $.post('http://Unique_Phone/CallContact', JSON.stringify({
        ContactData: cData,
...
$(document).on('click', ".phone-keypad-key-call", function(e){
    ...
    var phoneInput = $(".phone-keypad-input");
    if (phoneInput.length) {
       
        var inputText = phoneInput.text();
        var InputNum = inputText ? inputText.toString() : "";
...
$(document).on('click', '#new-chat-phone', function(e){
    ...
    if (ContactData.number !== MI.Phone.Data.PlayerData.charinfo.phone) {
        ...
        setTimeout(function(){
            ...
            $.post('http://Unique_Phone/GetWhatsappChat', JSON.stringify({phone: ContactData.number}), function(chat){
...
$(document).on('click', '#edit-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('http://Unique_Phone/EditContact', JSON.stringify({
...
$(document).on('click', '#edit-contact-delete', function(e){
    ...
    $.post('http://Unique_Phone/DeleteContact', JSON.stringify({
        CurrentContactName: ContactName,
        CurrentContactNumber: ContactNumber,
        CurrentContactIban: ContactIban,
    }), function(PhoneContacts){
...
$(document).on('click', '.phone-keypad-key', function(e) {
    ...
    var $keypadInput = $(".phone-keypad-input");
    var $keypadInputById = $("#phone-keypad-input");
    
  
    if (!$keypadInput.length || !$keypadInputById.length) {
        console.error("عنصر ورودی یافت نشد!");
        return;
    }
    
    var currentText = $keypadInput.text();
    var currentTextById = $keypadInputById.text();
    
   
    if (!isNaN(PressedButton)) {
    
        $keypadInput.text(currentText + PressedButton);
        $keypadInputById.text(currentTextById + PressedButton);
    } 
    else if (PressedButton === "*") {
      
        if (!window.ClearNumberTimer) {
            
            window.ClearNumberTimer = setTimeout(function() {
                $keypadInput.text("");
                $keypadInputById.text("");
                window.ClearNumberTimer = null;
            }, 50);
        }
    }
    else if (PressedButton === "#") {

        $keypadInput.text(currentText.slice(0, -1));
        $keypadInputById.text(currentTextById.slice(0, -1));
    }
});


function initNumericInput() {
    const $input = $('#phone-keypad-input');
    ...
    $input.on('input', function() {
        ...
        const cleanedText = $(this).text().replace(/[^\d]/g, '');
        
        if ($(this).text() !== cleanedText) {
            $(this).text(cleanedText);
            
            const range = document.createRange();
            range.setStart(this.firstChild, Math.min(cursorPos, cleanedText.length));
            range.collapse(true);
...
$(document).on('click', '#add-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('http://Unique_Phone/AddNewContact', JSON.stringify({
        ...
        if (SelectedSuggestion !== null) {
            $.post('http://Unique_Phone/RemoveSuggestion', JSON.stringify({
...
$(document).on('click', '#outgoing-cancel', function(e){
    ...
    $.post('http://Unique_Phone/CancelOutgoingCall');
});

$(document).on('click', '#incoming-deny', function(e){
    e.preventDefault();

    $.post('http://Unique_Phone/DenyIncomingCall');
});

$(document).on('click', '#ongoing-cancel', function(e){
    e.preventDefault();
    
    $.post('http://Unique_Phone/CancelOngoingCall');
});
...
MI.Phone.Functions.SetupCurrentCall = function(cData) {
    if (cData.InCall) {
        ...
        $(".phone-currentcall-contact").html("with "+cData.TargetData.name || "Staff");
    } else {
...
$(document).on('click', '#incoming-answer', function(e){
    ...
    $.post('http://Unique_Phone/AnswerCall');
});
...
$(document).on('click', '.suggested-contact', function(e){
    ...
    $.post('https://Unique_Phone/GetWhatsappChats', JSON.stringify({}), function(chats){
        MI.Phone.Functions.LoadWhatsappChats(chats);
    });
    ...
    setTimeout(function(){
        ...
        $.post('https://Unique_Phone/GetWhatsappChat', JSON.stringify({phone: SuggestionData.number}), function(chat){
            MI.Phone.Functions.SetupChatMessages(chat, {
                name: SuggestionData.name,
                number: SuggestionData.number
            });
        });
